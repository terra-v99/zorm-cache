...
func (h *queryHandler) BeforeQuery() func(db *gorm.DB) {
	...
	return func(db *gorm.DB) {
		...
		logx.WithContext(ctx).Infof("[BeforeQuery-First] traceId[%s]  CacheLevel[%+v],tableName[%+v],db.Statement.Table[%+v],db.Statement.Schema[%+v]",
			gtrace.GetTraceID(ctx), cache.Config.CacheLevel, tableName, db.Statement.Table, db.Statement.Schema)
		...
		if util.ShouldCache(tableName, cache.Config.Tables) {
			...
			tryBetsCache := func() (hit bool) {
				logx.WithContext(ctx).Infof("[BeforeQuery] try tryBetsCache! first")
				expr, err := sqlparser.NewParser(strings.NewReader(sql)).ParseStatement()
				if err != nil {
					if !errors.Is(err, storage.ErrCacheNotFound) {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Bet] get cache value for sql %s error: %v", sql, err)
					}
					db.Error = nil
					return
				}
				logx.WithContext(ctx).Infof("[BeforeQuery] try tryBetsCache! second")
				var condition sqlparser.Expr
				//var table *sqlparser.TableName
				switch stmt := expr.(type) {
				case *sqlparser.SelectStatement:
					logx.WithContext(ctx).Infof("[BeforeQuery] try tryBetsCache! 3")
					...
					logx.WithContext(ctx).Infof("[BeforeQuery] try tryBetsCache! 4")
					...
					logx.WithContext(ctx).Infof("[BeforeQuery-Bet] get value: %s", cacheValue)
					if cacheValue == "recordNotFound" { // 应对缓存穿透
						db.Error = util.RecordNotFoundCacheHit
						//TODO: 现已除排查bug
						// hit = true
					...
					if len(cacheValue) <= 2 {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Bet] unmarshal rows affected cache error: %v", err)
						db.Error = nil
						return
					}
					db.RowsAffected = 1
					...
					if err != nil {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Bet] unmarshal search cache error: %v", err)
						db.Error = nil
						return
					}
					logx.WithContext(ctx).Info("[BeforeQuery-Bet] tryBetsCache hit cache")
					db.Error = util.SearchCacheHit
					hit = true
					return
			...
			tryBetsDetailsCache := func() (hit bool) {
				...
				if err != nil {
					if !errors.Is(err, storage.ErrCacheNotFound) {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Detail] get cache value for sql %s error: %v", sql, err)
					}
					db.Error = nil
					return
				}
				...
				case *sqlparser.SelectStatement:
					...
					logx.WithContext(ctx).Infof("[BeforeQuery-Detail] get value: %s", cacheValue)
					if cacheValue == "recordNotFound" { // 应对缓存穿透
						db.Error = util.RecordNotFoundCacheHit
						hit = true
						return
					...
					if len(cacheValue) <= 2 {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Detail] unmarshal rows affected cache error: %v", err)
						db.Error = nil
						return
					}
					db.RowsAffected = 1
					err = iterJson.Unmarshal([]byte(cacheValue), db.Statement.Dest)
					if err != nil {
						logx.WithContext(ctx).Errorf("[BeforeQuery-Detail] unmarshal search cache error: %v", err)
						db.Error = nil
						return
					}
					logx.WithContext(ctx).Info("[BeforeQuery-Detail] tryBetsDetailsCache hit cache")
					db.Error = util.SearchCacheHit
					hit = true
					return
			...
			logx.WithContext(ctx).Infof("[BeforeQuery-Second] traceId[%s] CacheLevel[%+v],tableName[%+v],db.Statement.Table[%+v],db.Statement.Schema[%+v]",
				gtrace.GetTraceID(ctx), cache.Config.CacheLevel, tableName, db.Statement.Table, db.Statement.Schema)

			if cache.Config.CacheLevel == config.CacheLevelAll || tableName == "win_betslips" {
				logx.WithContext(ctx).Infof("[BeforeQuery] try bet!")
				if !hit && tryBetsCache() {
					hit = true
					return
				}
